...
class plugin_server {
  ...
  public:
    plugin_server(std::string const &connection)
        : context(1), socket(context, ZMQ_PAIR), cancelled(false),
    ...
    void loop() {
        while (!cancelled) {
            ...
            msgpack::unpack(&request_message, request_string.data(),
                            request_string.size());
            ...
            try {
                ...
                if (call_function != call_map.end()) {
                    ...
                    throw std::runtime_error(
                        std::string("no such function : ").append(c.name));
                }
            }
...
void initialize_dispatcher() {
    call_map["add"] = [](std::vector<std::string> parameters) {
        ...
        try {
            ...
            msgpack::unpack(&argv, parameters.front().data(),
                            parameters.front().size());
...
void benchmark() {
    ...
    std::cout << "Data size: " << (sizeof(numbers) + count * sizeof(int)) /
                                      1048576. << " MB" << std::endl;
    for (int i = 0; i < 3; i++) {
        auto start = std::chrono::high_resolution_clock::now();
